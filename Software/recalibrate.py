...
def prob_rat_uncert(data, quantile=None, method='percentile', pr_est=None, distribution=scipy.stats.lognorm):
    """
    Compute uncertainties in prob-ratio

    :param data: samples of prob-ratios generated by bootstrapping. Dim 0 is over samples, Dim 1 over differnt prs.
    :param quantile: the quantiles wanted for the uncertainty estimate. Default is [0.05, 0.95]
    :param method: method to compute the uncertainty -- one of: (see Paciorek et al 2018 https://doi.org/10.1016/j.wace.2018.01.002)
    ...
    :return: uncertainties - a 2x & ks (either None for everything but 'distribution' method.
    """
    if quantile is None:
        quantile = [0.05, 0.95]
    ...
    if method == 'percentile':
        ...
        uncerts = 2 * pr_est - np.quantile(data, quantile, axis=0)
